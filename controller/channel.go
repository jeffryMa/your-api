...
// controller/channel.go 中添加以下函数

func TestProxyIP(c *gin.Context) {
	...
	// 发送请求到外部API以验证代理是否工作
	// 这里使用httpbin.org，它会返回请求信息
	testURL := "https://ipconfig.io"
	resp, err := client.Get(testURL)
	if err != nil {
		c.JSON(500, gin.H{
			"success": false,
			"message": "代理测试失败: " + err.Error(),
		})
		return
	}
	...
	c.JSON(200, gin.H{
		"success": true,
		"message": "代理可用,当前测试代理ip:" + string(body), "data": gin.H{
			"proxy":       proxyURL,
			"response":    body,
