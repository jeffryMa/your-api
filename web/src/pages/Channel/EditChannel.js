...
function type2secretPrompt(type) {
  ...
  switch (type) {
    ...
    default:
      return '请输入渠道对应的鉴权密钥,多个秘钥使用英文逗号分隔';
  }
}
...
const EditChannel = (props) => {
  ...
  const handleInputChange = (name, value) => {
    if (name === 'key' && typeof value === 'string' && value.includes('，')) {
      showWarning(t('检测到中文逗号，已自动转换为英文逗号'));
    ...
    <>
      ...
      >
        <Spin spinning={loading}>
          ...
          />
          <div style={{ marginTop: 10 }}>
            <Typography.Text strong>{t('代理IP')}：</Typography.Text>
          </div>
          <Input
            name='proxy'
